...
def process_packet(packet):
    if packet.haslayer(Dot11FCS):
        print(' [ ' +  str(datetime.datetime.now())+ ' ] '+  ' DoS PS-Poll Attack Detected Against Mac Address: ' +   str(packet.addr2).swapcase())
#Running scanner for packet
sniff(iface="wlan1", prn=process_packet, store=False, count=0) #internal wi-fi card
